...
const FormPlanner = () => {
  ...
  // State untuk Susunan Acara (Timeline Jam)
  const [agendas, setAgendas] = useState([]);
  
  // State untuk Agenda Cadangan (Plan A, Plan B)
  ...
  // --- LOGIKA AGENDA CADANGAN (PLAN A, B, C) ---
  const addBackupPlan = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const { error } = await supabase.from('planners').insert([
        {
          ...
          status: 'planned',
          sub_plans: agendas, // Disimpan ke sub_plans
          backup_plans: backupPlans // Disimpan ke backup_plans
        }
      ]);
      ...
      console.error("Gagal menyimpan rencana:", error.message);
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-[#2A3A6A] p-6 font-sans pb-24 selection:bg-[#D85482]/30">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
           <h1 className="text-xl font-bold tracking-wide">Rencana Baru</h1>
           <p className="text-xs text-[#D85482] font-semibold uppercase tracking-wider">Detail Kencan</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="flex flex-col gap-6">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <input required type="text" name="title" value={formData.title} onChange={handleInputChange} placeholder="ex: Jalan-jalan ke Pantai" className="w-full bg-[#FAFAFA] border border-gray-200 rounded-xl px-4 py-3.5 font-bold focus:outline-none focus:border-[#D85482] transition" />
            </div>
            ...
            <div>
              ...
              <input type="text" name="location" value={formData.location} onChange={handleInputChange} placeholder="ex: Pantai Pasir Putih" className="w-full bg-[#FAFAFA] border border-gray-200 rounded-xl px-4 py-3.5 font-medium focus:outline-none focus:border-[#D85482] transition" />
            </div>
            ...
            <div>
              ...
              <textarea name="map_iframe" value={formData.map_iframe} onChange={handleInputChange} placeholder="Paste kode <iframe src='...'> dari Google Maps" rows="2" className="w-full bg-[#FAFAFA] border border-gray-200 rounded-xl px-4 py-3.5 text-xs text-gray-500 focus:outline-none focus:border-[#D85482] transition resize-none"></textarea>
            </div>
            ...
            <div>
              ...
              <textarea name="description" value={formData.description} onChange={handleInputChange} placeholder="Jangan lupa bawa kacamata hitam..." rows="2" className="w-full bg-[#FAFAFA] border border-gray-200 rounded-xl px-4 py-3.5 text-sm focus:outline-none focus:border-[#D85482] transition resize-none"></textarea>
            </div>
          </div>
        </div>

        {/* 2. SUSUNAN ACARA (TIMELINE) */}
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
              ...
              agendas.map((ag, idx) => (
                <div key={ag.id} className="flex gap-3 items-start bg-[#FAFAFA] p-3.5 rounded-xl border border-gray-100 relative">
                  ...
                  <div className="flex-1 flex flex-col gap-3">
                    ...
                    {/* Input Kegiatan */}
                    <input type="text" value={ag.activity} onChange={(e) => updateAgenda(ag.id, 'activity', e.target.value)} placeholder="Kegiatan (ex: Berangkat, Makan Siang)" className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2.5 text-sm font-bold focus:outline-none focus:border-[#2A4480]" />
                    
                    {/* Input Lokasi Khusus Jam ini */}
              ...
              backupPlans.map((bp, idx) => (
                <div key={bp.id} className="flex gap-3 items-start bg-white p-3.5 rounded-xl border border-[#D85482]/20 relative shadow-sm">
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                    <input type="text" value={bp.title} onChange={(e) => updateBackupPlan(bp.id, 'title', e.target.value)} placeholder="Nama Plan (ex: Nonton Netflix di Rumah)" className="w-full bg-[#FAFAFA] border border-gray-100 rounded-lg px-3 py-2 text-sm font-bold focus:outline-none focus:border-[#D85482]" />
                    <textarea value={bp.description} onChange={(e) => updateBackupPlan(bp.id, 'description', e.target.value)} placeholder="Detail plan cadangan..." rows="2" className="w-full bg-[#FAFAFA] border border-gray-100 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#D85482] resize-none"></textarea>
                  </div>
        ...
        <button type="submit" disabled={isSubmitting} className={`w-full text-white font-bold py-4 rounded-xl transition shadow-lg flex justify-center items-center gap-2 ${isSubmitting ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#2A4480] hover:bg-[#1f3360] active:scale-95'}`}>
          {isSubmitting ? <><div className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-white"></div> Menyimpan...</> : 'Simpan Rencana'}
        </button>
